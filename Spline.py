...
def spline_integrate(x, y, x1, x2):
    ...
    if i1 == i0:  # x1 i x2 se nalaze na istom segmentu
        tp = x1 - x[i0]
        tk = x2 - x[i0]
        A = a[i0] * (tk - tp) + b[i0] / 2 * (tk ** 2 - tp ** 2) + c[i0] / 3 * (tk ** 3 - tp ** 3) + d[i0] / 4 * (
                    tk ** 4 - tp ** 4)
    elif i1 - i0 == 1:  # x1 i x2 se nalaze na susednim intervalima
        # prvi segment
        tp = x1 - x[i0]
        tk = x[i1] - x1
        A = a[i0] * (tk - tp) + b[i0] / 2 * (tk ** 2 - tp ** 2) + c[i0] / 3 * (tk ** 3 - tp ** 3) + d[i0] / 4 * (
        ...
        tp = 0
        tk = x2 - x[i1]
        A += a[i1] * (tk - tp) + b[i1] / 2 * (tk ** 2 - tp ** 2) + c[i1] / 3 * (tk ** 3 - tp ** 3) + d[i1] / 4 * (
                    tk ** 4 - tp ** 4)
        # poslednj segment
    else:  # izmedju x1 i x2 postoji vise celih segmenata
        # prvi segment
        tp = x1 - x[i0]
        tk = x[i0 + 1] - x1
        A = a[i0] * (tk - tp) + b[i0] / 2 * (tk ** 2 - tp ** 2) + c[i0] / 3 * (tk ** 3 - tp ** 3) + d[i0] / 4 * (
        ...
        for i in range(i0 + 1, i1):
            tp = 0
            tk = x[i + 1] - x[i]
            A += a[i] * (tk - tp) + b[i] / 2 * (tk ** 2 - tp ** 2) + c[i] / 3 * (tk ** 3 - tp ** 3) + d[i] / 4 * (
                        tk ** 4 - tp ** 4)
...
def inverse_interp(x, y, y0):
    ...
    for j in range(len(y0)):
        xx = []
        for i in range(len(a)):
            t = np.roots([d[i], c[i], b[i], a[i] - y0[j]])  # trazi nule polinoma
...
def extrema(x, y, minmax):
    ...
    # =============================================================================
    a, b, c, d = spline_coefs(x, y)
    x0 = np.array([])  # mesta na kojima je prvi izvod jednak nuli
    d2 = np.array([])  # vrednost drugog izvoda
